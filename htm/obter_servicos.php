...
if (isset($_SESSION['id_user'])) {
    ...
    $sql = "SELECT nome_user, tipo_user FROM tb_usuarios WHERE id_user = $id_usuario";
...
$sql = "SELECT u.nome_user, s.valor, s.nome_serv, s.foto FROM tb_servicos s
    ...
    <style>
        ...
        *{
        ...
        .rodape{
            ...
            text-align: center;
            padding: 32px;
            border-top: 0.4px solid rgb(27, 15, 0);
            position: absolute;
            bottom: 0;
            height: 2.5rem;
            width: 100%;
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Executando a consulta SQL com INNER JOIN e termo de pesquisa
    $sql = "SELECT u.id_user, u.nome_user, s.id_serv, s.valor, s.nome_serv, s.foto FROM tb_servicos s
    JOIN tb_usuarios u ON s.fk_user = u.id_user
    WHERE u.tipo_user = 'T' AND s.nome_serv LIKE ?";
    $stmt = $conexao->prepare($sql);
    // Adicionando % ao redor do termo de pesquisa para corresponder a qualquer parte do nome do serviço
